...
누리온/리눅스 사용법에 대한 정보는 KISTI 홈페이지([https://www.ksc.re.kr](https://www.ksc.re.kr/))의 자료실 내에 누리온 사용자 지침서 등을 참고 바랍니다.
...
* 사용자의 홈디렉토리 혹은 서브 디렉토리에서 MD Nastran R2.1을 사용하는 경우 “nastran inputfile"의 형태로 입력합니다.

### 2. 환경 설정

* Nastran 실행 관련 설정을 변경하기 위해서는 환경설정 파일을 수정해야 합니다.
...
* 20161 버전 기본 환경 설정을 변경하고자 하는 경우, 위 경로의 "nast20182rc"를 홈 디렉토리나 작업 디렉터리로 복사하여 마침표를 파일이름에 추가하여 ".nast20182rc"로 파일 이름을 변경한 다음 그 파일에서 필요한 부분을 수정해서 사용 바랍니다.
...
※ 사용자 홈 디렉터리로 복사할 때 아래와 같이 사용하면 됩니다.
...
* 장시간의 계산 작업은 PBS 스케줄러를 이용하여 작업을 제출해야 합니다.
* <mark style="color:red;">작업 제출 전 "lic\_check" 명령을 실행하여 라이선스 상태를 확인 후 작업을 제출 바랍니다.</mark>

### 4. 스케쥴러 작업 스크립트 파일 작성

* 누리온 시스템에서는 로그인 노드에서 PBS 라는 스케쥴러를 사용하여 작업을 제출해야 합니다.
* 누리온 시스템에서 PBS 를 사용하는 예제 파일들이 아래의 경로에 존재하므로 사용자 작업용 파일을 만들 때 이를 참고 바랍니다.
...
* <mark style="color:red;">스크래치 디스크는 작업 종료 후 일정 시간이 지나면 이름이 변경되고 추후 삭제되기 때문에, 작업이 완료 된 경우 빠른 시일 내에 백업 권장합니다. (사용자 환경 > 사용자 파일시스템 및 쿼터 정책 참고)</mark>
* 기타 PBS 에 관련된 명령어 및 사용법은 [running-jobs-through-scheduler-pbs.md](../undefined/running-jobs-through-scheduler-pbs.md "mention")을 참고 바랍니다.&#x20;
...
{% hint style="info" %}
2022년 5월 11일에 마지막으로 업데이트 되었습니다.
{% endhint %}
