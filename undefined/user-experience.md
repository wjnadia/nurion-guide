...
* OTP 인증코드는 최초 아이디 발급시 자동으로 발급하여 안내(카카오톡 혹은 이메일)드립니다.
* 발급시 카카오톡(SMS) 혹은 이메일로 수신하신 "OTP 인증코드 발급 안내"를 참고하시기 바랍니다.
* 이후 휴대폰 교체 등의 사유로 재발급이 필요한 경우, 재발급사이트(https://otp.ksc.re.kr)을 이용해주시기 바랍니다.
...
```bash
$ ssh -l x123abc nurion.ksc.re.kr
(x123abc@nurion.ksc.re.kr) Password(OTP): OTP 6자리 숫자 토큰
(x123abc@nurion.ksc.re.kr) Password: 슈퍼컴퓨터 로그인 비밀번호
```

※ 리눅스 보안 정책상 입력하는 OTP 토큰 및 로그인 비밀번호는 화면에 표시되지 않습니다. 정확히 입력하신 후 엔터를 누르시기 바랍니다.\
※ OTP 토큰은 입력 실패시 최대 30초이후 변경된 새로운 토큰을 입력해야 합니다. (재사용 불가)\
※ 3회 실패시 누적 10분간 nurion 접근이 제한됩니다. 장시간 접근이 안되시면 상담(시스템 >> 시스템 접속)에 내용을 남겨주시기 바랍니다.
...
* 사용자 비밀번호를 변경하는 방법은 누리온에 접속 한 후 passwd 명령어를 통한 변경과 KSC 홈페이지를 통한 변경  2가지 방법이 제공됩니다.&#x20;
  * 장기 미사용으로 인한 접근 차단 및 패스워드 변경 기간(60일)이 지나서 누리온에 접속을 못 할 경우에는 KSC 홈페이지를 통해서 변경해야 합니다.

### &#x20;1. password 명령어를 통한 변경

* 사용자 패스워드를 변경하기 위해서는 누리온에 슈퍼컴퓨터 아이디를 통해 로그인 한 후 passwd 명령을 사용합니다.
...
### 2. 사용자 포털(www.ksc.re.kr)을 통한 변경

* 장기 미사용으로 인한 시스템 접속 불가 및  패스워드 변경 기간(60일)이 지났을 경우 비밀번호 변경은 사용자 포털(www.ksc.re.kr)을 통해서 가능합니다.
  * "마이페이지" -> "계정관리" ->"계정관리" -> 슈퍼컴 비밀번호 변경
* 변경된 비밀번호 동기화까지 다소 시간이 소요됩니다.
* OTP의 경우 180일 동안 미사용시 발급된 OTP가 폐기됩니다.
* 이 경우 OTP 재발급사이트(otp.ksc.re.kr)을 통해서 재발급이 가능합니다.
